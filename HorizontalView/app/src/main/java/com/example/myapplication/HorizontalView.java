...
public class HorizontalView  extends ViewGroup {
    ...
    public HorizontalView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(); }
    public HorizontalView(Context context, AttributeSet attrs, int defStyleAttr) { super(context, attrs, defStyleAttr);
        init(); }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (Math.abs(distance) > childWidth / 2) {  //判断左右滑动距离是否大于1/2
                    if (distance > 0) {
                        currentIndex++;
                    } else {
                        currentIndex--;
